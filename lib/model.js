 ...
 */
Model.filter = function(filters, quer, detail) {
  ...
  filters.forEach(function(alterableResponse) {
    Object.keys(alterableResponse).filter(function(potential) {
      return valid_alterables.contains(potential, quer, false);
    }).forEach(function(valid_key) {
      valid_alterables.filter(valid_key, alterableResponse[valid_key], quer);
    });
  });

  filters.forEach(function(filterableData) {
    Object.keys(filterableData).filter(function(potential_filter) {
      return valid_filters.contains(potential_filter, quer, detail);
    }).forEach(function(valid_key) {
      valid_filters.filter(valid_key, filterableData[valid_key], quer);
    });
  });
...
Model.preprocess = function(req, res, next) {
  ...
  req.body = req.body || {};
  req.query = req.query || {};
  req.quer = this.filter([req.query, req.headers], quer, detail);
  //req.quer = this.filter([req.body, req.query, req.headers], quer, detail);
  if (!('locals' in res)) {
    res.locals = {};
  }
  res.locals.bundle = {};
...
Model.detailRoute = function(req, res, next) {
  var method = req.method.toLowerCase();

  this.preprocess(req, res);
  this.routes[method].handler(req, res, next);
}
...
function filterable(props, subfilters) {
  return {
    ...
    contains: function(key, quer, ignoreProps) {
      if (props[key]) {
        if (ignoreProps) {
          return false;
        } else {
